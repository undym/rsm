...
export const FX_槍 = (center) => {
    ...
    const addBubble = () => {
        const rndRange = 0.04;
        const p = new Point(center.x + randomFloat(-rndRange, rndRange), center.y + randomFloat(-rndRange, rndRange));
        FX.add(count => {
            const over = 5;
...
export const FX_吸収 = (attacker, target) => {
    ...
    const addFX = (x, y) => {
        ...
        const createPoint = (count) => {
            return new Point((x * (over - count) + attacker.x * count) / over, (y * (over - count) + attacker.y * count) / over);
        };
        FX.add(count => {
    ...
    FX.add(count => {
        ...
        Graphics.setLineWidth(2, () => {
            for (const e of elms) {
                ...
                exists = true;
                const h = e.h * (1.0 - count / e.lifeTime) / 2;
                Graphics.line(new Point(e.x, e.y - h), new Point(e.x, e.y + h), Math.random() < 0.3 ? Color.WHITE : Color.D_RED);
                e.y += e.vy;
            }
        });
        return exists;
...
export const FX_反射 = (orner, refTarget) => {
    const over = 25;
    ...
    FX.add(count => {
        ...
        const points = [];
        for (let i = 0; i < num; i++) {
            points.push(new Point(refTarget.x + randomFloat(-xr, xr), refTarget.y + randomFloat(-yr, yr)));
        }
        Graphics.setLineWidth(Graphics.pixelW * 0.01, () => {
