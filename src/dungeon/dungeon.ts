...
export abstract class Dungeon{
    ...
    rndEvent():DungeonEvent{
        ...
        if(Math.random() < 0.03){
            ...
            if(this.rank >= 1){collectingEvents.push(DungeonEvent.STRATUM);}
            if(this.rank >= 2){collectingEvents.push(DungeonEvent.LAKE);}
            if(this.rank >= 3){collectingEvents.push(DungeonEvent.FOSSIL);}
...
export namespace Dungeon{
    ...
    export const                         魔水路:Dungeon = new class extends Dungeon{
        constructor(){super({uniqueName:"魔水路", info:"",
    ...
    export const                         ハデスの口:Dungeon = new class extends Dungeon{
        ...
        isVisible = ()=>Dungeon.小鬼.dungeonClearCount >= 1;
        setBossInner = ()=>{
            for(const e of Unit.enemies){
                for(const prm of Prm.atkPrms){
                    e.prm(prm).base *= 1.5;
    ...
    export const                         塔地下801階:Dungeon = new class extends Dungeon{
        constructor(){super({uniqueName:"塔地下801階", info:"",
        ...
        isVisible = ()=>Dungeon.塔地下782階.dungeonClearCount > 0;
        setBossInner = ()=>{
            const e = Unit.enemies[0];
