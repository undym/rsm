...
export const FX_暗黒 = (center:{x:number, y:number})=>{
    const addParticle = (point:{x:number, y:number}, vec:number, size:number, color:Color)=>{
        const over = 12;
        const rad = Math.PI * 2 * Math.random();
        const v = vec;
...
export const FX_過去 = (attacker:Point, target:Point)=>{
    FX.add((count)=>{
        const over = 20;
        const color = {r:0,g:0,b:0,a:1};
        const rndColor = ()=>{
        ...
        {//attacker-target
            ...
            for(let i = 0; i < loop; i++){
                if(i < a){
                    points.push(new Point(
                        (target.x * i + attacker.x * (loop - i)) / loop,
                        (target.y * i + attacker.y * (a - i)) / a,
                    ));
                }else{
                    points.push(new Point(
                        (target.x * i + attacker.x * (loop - i)) / loop,
            ...
            for(let i = 1; i < points.length - 1; i++){
                ...
                const v_2 = v / 2;
                points[i] = new Point(
                    points[i].x - v_2 + Math.random() * v,
                    points[i].y - v_2 + Math.random() * v,
                );
            }
...
export const FX_ナーガ着弾 = (attacker:Point, target:Point)=>{
    const rnd = target.x - attacker.x;
    const x = attacker.x + rnd * Math.random();
    const center = new Point(x, 0);
    FX.add((count)=>{
...
export const FX_LVUP = (img:Img, bounds:Rect, transparence:Color, reverse:boolean)=>{
...
export const FX_BOM = (center:Point)=>{
    let r = 0.01;
    let vec = 0.03;
